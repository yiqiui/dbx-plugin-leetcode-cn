...
- **收藏星标**：每行末尾一颗 ⭐/☆，点击即收藏或取消；星标初值来自列表接口的 `isFavor` 字段（无需额外请求）。左侧「我的收藏」= 在当前结果中只看已收藏。
- **每日一题**：右侧栏与左侧导航都会显示当日题目（日期 / 难度 / 中文标题），点击直接进入做题页。
- **热门企业题库 / 官网板块**：右侧企业列表与左侧「探险模式 / LeetBook / 学习计划」通过 sidecar 用系统浏览器打开对应的 leetcode.cn 页面（这些板块没有稳定的公开接口，跳转官网最可靠）。
- **登录态显示**：顶部展示当前昵称与用户名，并带一枚首字母头像徽章（沙箱 CSP 不允许加载站外图片，因此不用远程头像 URL）。
...
node dev/ui.test.mjs      # DOM 级交互回归：页签/标签展开收起/排序方向/收藏星标/随机一题/企业跳转/筛选组合/重置
```
...
- **收藏的写入路径未经线上验证**：星标**读取**用列表接口的 `isFavor`（已验证）；点击收藏走的 mutation 名称无法在未登录状态下确认（对 `Mutation` 的探测会被反爬拦截），因此若接口拒绝，界面会把真实错误显示在状态行而不是静默失败。用登录会话实测后可能需要校正该调用。
...
- **头像用首字母徽章**：工作台 CSP 为 `default-src 'none'`，站外图片无法加载，因此不请求 leetcode 的头像 URL。
- **探险模式 / LeetBook / 学习计划 / 企业题库跳系统浏览器**：这些板块没有稳定的公开接口，插件内复刻既不可靠也难维护。
